...
from models import Eco_Scope2Co2, EcoflocResult
...
class Scope2ResultOut(BaseModel):
    id: int
    process_name: str
    resource_type: str
    co2_kg: float
...
def ingest_scope2(payload: Scope2IngestRequest, db: Session = Depends(get_db)):
    try:
        results = ingest_scope2_from_ecofloc(
            db=db,
            carbon_intensity_g_per_kwh=payload.carbon_intensity,
